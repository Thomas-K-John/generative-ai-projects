...
- Once the application server startup is complete, go to the Swagger UI provided by FastAPI:
  ...
  • We are using a hybrid approach combining vector search and keyword search, followed by reranking the retrieved chunks.
  • ThreadPoolExecutor: runs blocking code in threads to allow parallel processing in async FastAPI.
  ...
  • While RAGAS is used for response validation, certain queries require corrections to their ground truth references.
